...
def load_elab_registry(
    registry,
    server=None,
    token=None,
    category="Plasmids",
    include_tags=frozenset({"YTK"}),
    exclude_tags=None,
):
...
def ask_parts(registry, qgrid_plasmids):
    ...
    for colname, part_type in types.items():
        ...
        parts[colname] = pandas.Categorical(
            [""] * len(names), categories=[""] + sorted(categories)
        )
...
def validate_assemblies(registry, qgrid_parts):
    ...
    for pid, parts in assemblies.items():
        ...
        vector = parts.pop(
            next(
                ...
                if isinstance(parts[i], ytk.YTKCassetteVector)
            )
        )
    ...
    display(
        df_full.drop(columns=["Record", "Typed part"])
...
def ask_concentrations(assemblies):
    ...
    df_strains = (
        assemblies.drop(columns=["Plasmid Name", "Plasmid ID"])
    ...
    concentration_widget = qgrid.QGridWidget(
        df=df_strains[["Part ID", "Sample concentration (ng/µL)"]]
    )
...
def dilution_table(df_strains, concentration_widget):
    ...
    ] = concentration_widget.get_changed_df().sort_index()[
        "Sample concentration (ng/µL)"
    ]
    df_strains["MW (Dalton)"] = df_strains["Record"].apply(
        lambda r: Bio.SeqUtils.molecular_weight(r.seq, "DNA", True, True)
    )
    df_strains["Sample concentration (fmol/µL)"] = (
        10 ** 6 * df_strains["Sample concentration (ng/µL)"] / df_strains["MW (Dalton)"]
    )
    df_strains["µL of sample to 40fmol"] = (
        20 / df_strains["Sample concentration (fmol/µL)"]
    )
    df_strains["Dilution factor to 20fmol/µL"] = round(
        1 / df_strains["µL of sample to 40fmol"]
    ).apply(int)
...
def generate_protocol(strains, assemblies):
    ...
    for idx, plasmid_id in enumerate(assemblies["Plasmid ID"].unique()):
        ...
        for _, part in parts.iterrows():
            dilution = strains[strains["Part ID"] == part["Part ID"]][
                "Dilution factor to 20fmol/µL"
            ]
            tube = strains[strains["Part ID"] == part["Part ID"]].index[0]
            protocol.append("<li>")
            protocol.append(
                "{} - Strain {} (diluted {} time·s)".format(
                    part["Part ID"], tube, int(dilution)
                )
            )
            protocol.append("</li>")
        ...
        protocol.append("</ul>")
        protocol.append(
            "Add {:.2f} µL of water (q.s.p 7.2 µL)".format(7.2 - len(parts))
        )
        protocol.append("</p>")
