...
class TestCircularRecord(unittest.TestCase):
    ...
    def test_shift_features(self):
        ...
        """
        ft = [
            SeqFeature(FeatureLocation(ExactPosition(0), ExactPosition(2), strand=+1), type='promoter'),
            SeqFeature(FeatureLocation(ExactPosition(2), ExactPosition(4), strand=+1), type='promoter')
        ]
        sr = SeqRecord(seq=Seq('ATGC'), id='feats', features=ft)
        cr = CircularRecord(sr)

        cr_1 = cr >> 1
        self.assertEqual(cr_1.features[0].location, FeatureLocation(ExactPosition(1), ExactPosition(3), strand=+1))
        self.assertEqual(cr_1.features[1].location, FeatureLocation(ExactPosition(3), ExactPosition(5), strand=+1))

        cr_2 = cr_1 >> 1
        self.assertEqual(cr_2.features[0].location, FeatureLocation(ExactPosition(2), ExactPosition(4), strand=+1))
        self.assertEqual(cr_2.features[1].location, FeatureLocation(ExactPosition(0), ExactPosition(2), strand=+1))

        cr_3 = cr_2 >> 1
        self.assertEqual(cr_3.features[0].location, FeatureLocation(ExactPosition(3), ExactPosition(5), strand=+1))
        self.assertEqual(cr_3.features[1].location, FeatureLocation(ExactPosition(1), ExactPosition(3), strand=+1))
